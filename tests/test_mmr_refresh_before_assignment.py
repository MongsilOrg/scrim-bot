...
버그: ScrimOrchestrator._refresh_mmr_before_assignment 가 존재하지 않는
`update_all_team_mmr` (언더스코어 없음)를 호출해 매번 AttributeError 발생 →
broad except 로 삼켜지고 MMR 갱신/타임스탬프가 전혀 반영되지 않음.
"""
import unittest
...
MANAGER_ATTRS = [
    'teams', '_update_all_team_mmr', 'update_mmr_message',
    'mmr_message', 'scrim_channel_id', 'client', '_last_success_time',
]
...
class TestRefreshMmrBeforeAssignment(unittest.IsolatedAsyncioTestCase):
    def _make_manager(self, mmr_result):
        ...
        mgr.teams = {'팀A': object()}
        mgr._update_all_team_mmr = AsyncMock(return_value=mmr_result)
        mgr.update_mmr_message = AsyncMock()
        mgr.mmr_message = None
        mgr.scrim_channel_id = 123
        client = MagicMock()
        client.get_channel.return_value = object()
        mgr.client = client
        mgr._last_success_time = "16:55"
        return mgr

    async def test_refresh_updates_last_success_time_on_success(self):
        """직전 갱신 성공 시 마지막 갱신 시각이 17:00 으로 반영돼야 함"""
        mgr = self._make_manager((3, 0))
        orch = ScrimOrchestrator(MagicMock())
        with patch('models.scrim_orchestrator.get_current_kst_time',
                   return_value=datetime(2026, 6, 1, 17, 0)):
            await orch._refresh_mmr_before_assignment(mgr)
        self.assertEqual(mgr._last_success_time, "17:00")
        mgr._update_all_team_mmr.assert_awaited_once_with(force=True)
        mgr.update_mmr_message.assert_awaited_once()

    async def test_refresh_keeps_time_when_all_fail(self):
        """전체 실패 시 기존 마지막 성공 시각(16:55) 유지"""
        mgr = self._make_manager((0, 5))
        orch = ScrimOrchestrator(MagicMock())
        with patch('models.scrim_orchestrator.get_current_kst_time',
                   return_value=datetime(2026, 6, 1, 17, 0)):
            await orch._refresh_mmr_before_assignment(mgr)
        self.assertEqual(mgr._last_success_time, "16:55")
        mgr.update_mmr_message.assert_awaited_once()
...
class TestUpdateAllTeamMmrForce(unittest.IsolatedAsyncioTestCase):
    ...
    async def _run(self, force):
        ...
        mgr = MagicMock(spec=['teams', 'set_team_mmr', '_save_backup'])
        mgr.teams = {'팀A': team}
        mgr.set_team_mmr = AsyncMock()
        mgr._save_backup = MagicMock()
        ...
        with patch('models.mmr_updater.get_current_kst_time',
                   return_value=datetime(2026, 6, 1, 17, 0)), \
             patch('bot.manager.BotManager') as BM:
