...
class TeamInputView(View):
    ...
    async def add_team_callback(self, interaction: discord.Interaction) -> None:
        ...
        try:
            ...
            if team_data_manager.is_team_assignment_started:
                await self._send_error_message(interaction, "조 편성이 이미 시작되어 팀 등록이 불가능합니다.\n\nGroup assignment has already started. Team registration is no longer available.")
                return
            ...
            if user_team:
                ...
                if not interaction.response.is_done():
                    ...
                    await interaction.followup.send("모달을 표시할 수 없습니다. 새로 시도해주세요. / Unable to show modal. Please try again.", ephemeral=True)
                    
        except Exception as e:
            logger.error(f"[뷰] 팀 추가 콜백 처리 실패: {e}", exc_info=True)
            await self._send_error_message(interaction, "팀 추가 중 오류가 발생했습니다. / An error occurred while adding team.")
    ...
    async def cancel_team_callback(self, interaction: discord.Interaction) -> None:
        ...
        try:
            ...
            if not user_team:
                await self._send_error_message(interaction, "등록한 팀이 없습니다. / No registered team.")
                return
            
            # 팀 취소 처리
            await self._process_team_cancellation(interaction, user_team)
            
        except Exception as e:
            logger.error(f"[뷰] 팀 취소 콜백 처리 실패: {e}", exc_info=True)
            await self._send_error_message(interaction, "팀 취소 중 오류가 발생했습니다. / An error occurred while cancelling team.")
    ...
    async def _show_team_edit_modal(self, interaction: discord.Interaction, team_name: str, team_data_manager) -> None:
        ...
        try:
            ...
            if not team_data:
                await self._send_error_message(interaction, "팀 정보를 찾을 수 없습니다. / Team information not found.")
                return
            ...
            await self._send_error_message(interaction, "팀 수정 모달 표시 중 오류가 발생했습니다. / An error occurred while showing team edit modal.")
    ...
    async def admin_log_callback(self, interaction: discord.Interaction) -> None:
        ...
        try:
            ...
            if not self._is_admin(interaction.user):
                error_embed = Embed(
                    title="스크림 안내",
                    description="관리자 권한이 없습니다 / Admin access required",
                    color=Color.red()
                )
                # 상호작용이 이미 응답되었는지 확인
                if not interaction.response.is_done():
            ...
            await self._send_error_message(interaction, "로그 조회 중 오류가 발생했습니다. / An error occurred while viewing logs.")
    ...
    async def _process_team_registration(self, interaction: discord.Interaction, team_name: str, team_data: dict, temp_message: discord.Message = None) -> None:
        ...
        try:
            ...
            if team_data_manager.is_team_assignment_started:
                if temp_message:
                    await self._update_temp_message(temp_message, "조 편성이 이미 시작되어 팀 등록이 불가능합니다.\n\nGroup assignment has already started. Team registration is no longer available.", discord.Color.red())
                else:
                    await self._send_error_message(interaction, "조 편성이 이미 시작되어 팀 등록이 불가능합니다.\n\nGroup assignment has already started. Team registration is no longer available.")
                return
            ...
            if not has_test_account:
                ...
                try:
                    ...
                    if temp_message:
                        await self._update_temp_message(temp_message, "닉네임 확인 중 문제가 발생했습니다. 잠시 후 다시 시도해주세요. / Error verifying nickname. Please try again later.", discord.Color.red())
                    else:
                        await self._send_error_message(interaction, "닉네임 확인 중 문제가 발생했습니다. 잠시 후 다시 시도해주세요. / Error verifying nickname. Please try again later.")
                    return
            ...
            try:
                ...
                team_data_manager.set_team_mmr(team_name, team_mmr)
            except Exception as e:
            ...
            if not success:
                ...
                error_message = failure_reason if failure_reason else (
                    ...
                    "💡 신청 시간 제한 또는 예비팀 모집 제한을 확인해주세요.\n"
                    "Please check the registration time limit or spare team limit."
                )
                if temp_message:
                    await self._update_temp_message(temp_message, error_message, discord.Color.red())
                else:
            ...
            # 성공 메시지 처리
            logger.info(f"[팀등록] 팀 등록 완료 - 팀명: {team_name}, MMR: {team_mmr:.2f}, 플레이어: {len(players)}명")
            
            success_msg = f"**{team_name}** 팀이 성공적으로 등록되었습니다!\nTeam **{team_name}** has been successfully registered!\n\n팀 평균 MMR / Team Average MMR: **{team_mmr:.2f}**"
            ...
            if temp_message:
                ...
                # 기존 방식으로 성공 메시지 전송
                success_embed = Embed(
                    title="스크림 팀 등록 완료 / Team Registration Complete",
    ...
    async def _update_temp_message(self, temp_message: discord.Message, message: str, color: discord.Color) -> None:
        ...
        try:
            embed = discord.Embed(
                title="스크림 팀 등록 완료 / Team Registration Complete" if color == discord.Color.green() else "스크림 안내 / Scrim Notice",
                description=message,
                color=color
            )
    ...
    async def _process_team_cancellation(self, interaction: discord.Interaction, team_name: str) -> None:
        ...
        try:
            ...
            # 조편성 시작 이후인지 확인
            if team_data_manager.is_team_assignment_started:
                await self._send_error_message(interaction, "조 편성이 이미 시작되어 팀 취소가 불가능합니다.\n\nGroup assignment has already started. Team cancellation is no longer available.")
                return
            ...
            if team_name not in team_data_manager.get_all_teams():
                await self._send_error_message(interaction, "등록되지 않은 팀명입니다. / Team name not registered.")
                return
    ...
    async def _execute_team_cancellation(self, interaction: discord.Interaction, team_name: str, team_data_manager: TeamDataManager) -> None:
        ...
        try:
            ...
            if not success:
                ...
                error_message = failure_reason if failure_reason else (
                    "❌ 팀 취소가 실패했습니다.\n\n"
                    "Team cancellation failed.\n\n"
                    "💡 취소 시간 제한을 확인해주세요.\n"
                    "Please check the cancellation time limit."
                )
                error_embed = Embed(
                    title="❌ 팀 취소 실패 / Team Cancellation Failed",
            ...
            # 성공 메시지 전송
            success_embed = Embed(
                title="스크림 팀 등록 취소 / Team Registration Cancelled",
                description=f"**{team_name}** 팀이 성공적으로 취소되었습니다.\nTeam **{team_name}** has been successfully cancelled.",
                color=Color.green()
            )
            ...
            error_embed = Embed(
                title="❌ 오류 발생 / Error Occurred",
                description="팀 취소 중 오류가 발생했습니다. / An error occurred while cancelling team.",
                color=Color.red()
            )
            if not interaction.response.is_done():
                await interaction.response.send_message(embed=error_embed, ephemeral=True)
    ...
    def _create_log_embed(self) -> discord.Embed:
        ...
        if logs["신청"]:
            ...
            log_text = "\n".join(application_logs) if application_logs and isinstance(application_logs, (list, tuple)) else "없음 / None"
            # 1024자 제한 적용
        ...
        if logs["취소"]:
            ...
            log_text = "\n".join(cancellation_logs) if cancellation_logs and isinstance(cancellation_logs, (list, tuple)) else "없음 / None"
            # 1024자 제한 적용
            if len(log_text) > 1024:
                log_text = log_text[:1021] + "..."
            embed.add_field(
                name="❌ 취소 (Cancellation)",
                value=log_text,
                inline=False
            )
        ...
        if logs["수정"]:
            ...
            log_text = "\n".join(modification_logs) if modification_logs and isinstance(modification_logs, (list, tuple)) else "없음 / None"
            # 1024자 제한 적용
            if len(log_text) > 1024:
                log_text = log_text[:1021] + "..."
            embed.add_field(
                name="✏️ 수정 (Modification)",
                value=log_text,
                inline=False
            )
    ...
    async def _send_log_as_file(self, interaction: discord.Interaction) -> None:
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(mode='w', encoding='utf-8', suffix='.txt', delete=False) as f:
                f.write("=" * 60 + "\n")
                f.write("스크림 로그 / Scrim Log\n")
                f.write("=" * 60 + "\n\n")
                
                # 신청 로그
                f.write("📝 신청 (Application)\n")
                f.write("-" * 60 + "\n")
                if logs["신청"]:
                    for log in logs["신청"]:
            ...
            if file_size > 25 * 1024 * 1024:  # 25MB
                ...
                error_embed = Embed(
                    title="❌ 오류 / Error",
                    description="로그 파일이 너무 큽니다. (25MB 초과)\nLog file is too large. (exceeds 25MB)",
                    color=Color.red()
                )
                if not interaction.response.is_done():
                    await interaction.response.send_message(embed=error_embed, ephemeral=True)
            ...
            info_embed = Embed(
                title="📋 스크림 로그 / Scrim Log",
                description="로그가 너무 길어 텍스트 파일로 전송합니다.\nLog is too long, sending as text file.",
                color=Color.blue()
            )
    ...
    async def _send_error_message(self, interaction: discord.Interaction, message: str) -> None:
        ...
        try:
            ...
            try:
                ...
                logger.error(f"[뷰] 최후의 수단 메시지 전송 실패: {e2}", exc_info=True)
...
class GroupRosterView(View):
    ...
    async def roster_change_callback(self, interaction: discord.Interaction) -> None:
        ...
        try:
            ...
            embed = Embed(
                title="팀 선택 / Select Team",
                description="변경할 팀을 선택하세요. / Please select the team to modify.",
                color=Color.blue()
            )
            ...
            await self._send_error_message(interaction, "로스터 변경 중 오류가 발생했습니다. / An error occurred while changing roster.")
...
class TeamSelectionView(View):
    ...
    def __init__(self, parent_view: 'GroupRosterView'):
        ...
        self.team_select = Select(
            placeholder="변경할 팀을 선택하세요 / Select team to modify",
            options=[
                SelectOption(
                    label=f"{i+1}. {team_name} (MMR: {mmr:.2f})",
                    value=team_name,
                    description=f"팀원 / Team Members: {', '.join(team_data.players[:3]) if hasattr(team_data, 'players') and team_data.players and isinstance(team_data.players, (list, tuple)) else ', '.join(team_data.get('players', [])[:3]) if isinstance(team_data, dict) and team_data.get('players') and isinstance(team_data.get('players'), (list, tuple)) else '정보 없음 / No info'}"
                )
    ...
    async def team_select_callback(self, interaction: discord.Interaction) -> None:
        ...
        try:
            ...
            if not selected_team_data:
                error_embed = Embed(
                    title="오류 / Error",
                    description="선택된 팀 정보를 찾을 수 없습니다. / Selected team information not found.",
                    color=Color.red()
                )
                if not interaction.response.is_done():
                    await interaction.response.send_message(embed=error_embed, ephemeral=True)
            ...
            error_embed = Embed(
                title="오류 / Error",
                description="팀 선택 중 오류가 발생했습니다. / An error occurred while selecting team.",
                color=Color.red()
            )
            try:
                if not interaction.response.is_done():
    ...
    async def import_csv_callback(self, interaction: discord.Interaction) -> None:
        ...
        try:
            ...
            if not interaction.response.is_done():
                ...
                await interaction.followup.send("모달을 표시할 수 없습니다. 새로 시도해주세요.", ephemeral=True)
